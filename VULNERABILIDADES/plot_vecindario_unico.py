...
# Lista de nodos con vecindario único según tu análisis
vecindario_unico = ['A','B','C','D','E','F','G','N','T','U','V','W','X']

plt.figure(figsize=(10,7))
pos = nx.spring_layout(G, seed=42)
# Color rojo para los nodos de vecindario único, azul para los demás
colores = ['red' if n in vecindario_unico else 'lightblue' for n in G.nodes()]
nx.draw(G, pos, with_labels=True, node_color=colores, node_size=400, edge_color='gray', font_size=10)
plt.title('Nodos con vecindario único (rojo)')
plt.tight_layout()
plt.savefig('VECINDARIO_UNICO.png', dpi=150)
plt.show()
print("Imagen guardada como 'VECINDARIO_UNICO.png'")